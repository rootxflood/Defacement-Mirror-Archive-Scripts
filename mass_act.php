...
$kontrol = $db->numRows();
if ($kontrol > 0) {
    zeaTambahPesan("Domain has been deface last year");
} else {
...
if ($content == "") {
    zeaTambahPesan("Domain Is Inactive/Blank Page");
} else if (cek_konten($content)) {
    zeaTambahPesan("Domain Hasn't Been Defaced !!");
} else {
    ...
    $regip = $_SERVER['REMOTE_ADDR'];
    $special = "0";
    if ((strstr($url, ".gov")) or (strstr($url, ".edu")) or (strstr($url, ".in")) or (strstr($url, ".pk")) or (strstr($url, ".gob")) or (strstr($url, ".edu")) or (strstr($url, ".mil")) or (strstr($url, "https://"))) {
        $special = "1";
    }
    ...
    if ($query && $query2 && $query3) {
        if ($status == 1) {
            zeaTambahPesan("Domain Added In Archives");
        }
        if ($status == 0) {
            zeaTambahPesan("Domain Added On Hold");
        }
    } else {
        zeaTambahPesan("Domain fails added. Please try again");
    }
	}
}
zeaRedirect('mass.notify');
?>